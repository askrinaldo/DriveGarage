...
/**
 * Register a getter that returns arbitrary extra headers to attach to every
 * fetch request.  Useful for injecting custom auth headers (e.g. x-user-token)
 * that are not covered by the standard Authorization bearer mechanism.
 * Pass `null` to clear the getter.
 */
...
export async function customFetch<T = unknown>(
  ...
  // Attach any extra headers supplied by the getter (e.g. x-user-token).
  ...
  // Always include credentials (cookies) so Clerk session cookies are forwarded
  // to the API server. This is safe for same-origin and same-site requests.
